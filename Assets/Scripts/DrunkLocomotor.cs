...
namespace Oculus.Interaction.Locomotion
{
    ...
    /// </summary>
    public class DrunkLocomotor : MonoBehaviour,
        ILocomotionEventHandler, IDeltaTimeConsumer, ITimeConsumer
    {
        [Header("Effets d'ébriété")]
        [Range(0f, 1f)]
        public float drunkenness = 0f;
        [Tooltip("Force de la déviation latérale. Plus c'est élevé, plus le joueur dévie.")]
        public float swayIntensity = 0.8f;
        [Tooltip("La vitesse à laquelle la déviation change de direction.")]
        public float swaySpeed = 0.3f;
        private float _perlinSeed;
        [Tooltip("La référence au script BalanceManager qui gère le balancement de la caméra.")]
        ...
        protected virtual void Start()
        {
            ...
            _perlinSeed = UnityEngine.Random.Range(0f, 100f);
        ...
        protected virtual void Update()
        {
            ...
            {
                UpdateVelocity();
                Vector2 primaryThumbstick = OVRInput.Get(OVRInput.Axis2D.PrimaryThumbstick, OVRInput.Controller.LTouch);

                // On ne s'active que si le joueur est ivre, au sol, et qu'il a une référence au BalanceManager.
                if (drunkenness > 0.01f && IsGrounded && balanceManager != null && primaryThumbstick.magnitude > 0.2f)
                {
                    ...
                    {
                        // On mémorise la vitesse d'origine voulue par le joueur
                        float originalSpeed = horizontalVelocity.magnitude;

                        // On calcule une force de déviation purement latérale
                        float offset = (Mathf.PerlinNoise(Time.time * swaySpeed, _perlinSeed) * 2f) - 1f;
                        Vector3 swayForce = -transform.right * offset * swayIntensity * drunkenness;

                        // On ajoute la déviation à la vélocité horizontale
                        Vector3 combinedVelocity = horizontalVelocity + swayForce;

                        // CRUCIAL : On normalise et on réapplique la vitesse d'origine
                        // pour ne pas accélérer ou ralentir le joueur.
                        Vector3 finalHorizontalVelocity = combinedVelocity.normalized * originalSpeed;

                        // On réassemble la vélocité finale avec la gravité
                        _velocity = finalHorizontalVelocity + (Vector3.up * verticalVelocity);
                    }
                }

                Pose startPose = _characterController.Pose;
                Vector3 delta = _velocity * _deltaTimeProvider.Invoke();
                _characterController.Move(_velocity * _deltaTimeProvider.Invoke());
